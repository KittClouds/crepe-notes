...
import { EntityKind, ENTITY_ICONS, getEntityColor, getEntityBgColor } from "@/lib/types/entityTypes";
import { getDisplayName, parseEntityFromTitle } from "@/lib/utils/titleParser";
// Stub BlueprintHub until wired
const useBlueprintHub = () => ({ toggleHub: () => { }, isHubOpen: false });
        ...
        {ENTITY_TYPES.map(({ kind, label }) => {
          ...
          return (
            <DropdownMenuItem
              ...
              className="gap-2"
            >
              <Icon className="h-4 w-4" style={{ color: getEntityColor(kind) }} />
              <span>{label}</span>
              <span
                className="ml-auto text-[10px] px-1.5 py-0.5 rounded font-medium"
                style={{
                  backgroundColor: getEntityBgColor(kind),
                  color: getEntityColor(kind),
                }}
              >
                {kind}
            ...
            >
              ...
              {note.isEntity && note.entityKind && (
                <span
                  ...
                  style={{
                    backgroundColor: getEntityBgColor(note.entityKind),
                    color: getEntityColor(note.entityKind),
                  }}
                >
