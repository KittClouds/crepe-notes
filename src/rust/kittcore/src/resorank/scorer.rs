...
//!
//! The core scoring engine implementing BM25F with proximity and BM𝒳 extensions.
...
/// ResoRank Scorer - BM25F with Proximity and BM𝒳 extensions
#[wasm_bindgen]
...
impl ResoRankScorer {
    ...
    fn score_single_term(&mut self, term: &str, doc_id: &str) -> F32 {
        ...
        if self.config.enable_bmx_similarity {
            if let Some(beta) = self.cached_beta {
                score += beta * 1.0 * 1.0; // β × S(Q,D) × E(qi) = β × 1 × 1
            }
        }
    ...
    pub fn explain_score(&mut self, query: &[String], doc_id: &str) -> ScoreExplanation {
        ...
        // Calculate query-level entropy if BM𝒳 enabled
        let entropy_stats =
...
impl ResoRankScorer {
    ...
    pub fn js_to_json(&self) -> Result<JsValue, JsValue> {
